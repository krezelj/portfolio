...
PARSE_DATA_FIRST = True

### Which data to save
TO_SAVE = {
    "track_r_data":     True,
    # "artist_r_data":    False,
    "track_scores":            False, # requires track_r_data and unique_track_names
    "track_scores_over_time":  False, # requires track_r_data and unique_track_names
    "track_scores_per_period": False  # requires track_r_data and unique_track_names
}
# ====================
#
#
...
def main():
    ...
    if PARSE_DATA_FIRST:
        if INITIALS[-1] == 'X':
            eshp = utils.ExtendedStreamHistoryParser("Data/Raw/{INITIALS}/", f"Data/Parsed/{INITIALS}_parsed_data.csv")
            eshp.parse()
