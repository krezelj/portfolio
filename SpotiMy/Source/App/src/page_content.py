...
PARSED_DATA_PATH_PREFIX="../../../Data/Parsed"
ANALYSED_DATA_PATH_PREFIX="../../../Data/Analysed"
ASSETS_PATH_PREFIX="../../../Source/App/assets"
...
class PageContent():
    ...
    def __init__(self) -> None:
        ...
        # self.SS = 'SSX'
        # self.SM = 'SMX'
        self.SS = 'JKX' # Change for portoflio purposes
        self.SM = 'JKX' # Change for portoflio purposes
    ...
    def __init_data(self):
        def get_data(initials):
            data = pd.read_csv(f"{PARSED_DATA_PATH_PREFIX}/{initials}_parsed_data.csv", sep=';')
            data["date"] = pd.to_datetime(data["date"]).dt.tz_localize(None)
            data = data.sort_values(by="date")
            return data
        def get_meta_data(initials):
            meta_data = pd.read_csv(f"{PARSED_DATA_PATH_PREFIX}/{initials}_meta_data.csv", ';')
            return meta_data
        def get_score_data(initials):
            data = pd.read_csv(f"{ANALYSED_DATA_PATH_PREFIX}/Score/{initials}_track_weekly.csv", sep=';')
            data = data.sort_values(by="score")
            return data
        def get_track_r_data(initials):
            r_data = pd.read_csv(f"{ANALYSED_DATA_PATH_PREFIX}/RelevanceData/{initials}_track_r_data.csv", sep = ';')
            r_data["date"] = pd.to_datetime(r_data["date"]).dt.tz_localize(None)
            track_names = []
            with open(f"{ANALYSED_DATA_PATH_PREFIX}/RelevanceData/{initials}_unique_track_names.txt", 'r', encoding='utf-8') as f:
    ...
    def __init_dropdowns(self):
        ...
        self.user_dropdown = CCDropdown(
            ...
            (self.JK, 'JAN KREZEL'),
            (self.SS, 'GROUP MEMBER 1'),
            (self.SM, 'GROUP MEMBER 2')])
            ...
            html.Div(id='created_by_div', 
                children=[
                    ...
                    html.P([
                        ...
                        html.Br(),
                        dcc.Link(children="Group Member 1", href="", 
                            id = 'SS_link', 
                            style={'margin': '0px', 'color': 'white', 'text-decoration': 'none'}),
                        html.Br(),
                        dcc.Link(children="Group Member 2", href="", 
                            id = 'SM_link', 
                            style={'margin': '0px', 'color': 'white', 'text-decoration': 'none'})], 
    ...
    def get_md_text(self, file_name):
        file_path = f"{ASSETS_PATH_PREFIX}/mdfiles/" + file_name
